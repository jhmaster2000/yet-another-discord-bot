export function run(client, message, args) {
    ...
    const position = args.options.get('position') || args.options.get('pos') || null;
    ...
    if (type === 'text' && isNaN(slowmode)) return; // Response on parseSlowmode() function
    ...
    const vc_bitrate = args.options.get('bitrate') || args.options.get('kbps');
    const vc_userlimit = args.options.get('userlimit') || args.options.get('users');
    if (type === 'voice') {
    ...
    message.guild.channels.create(name, {
        ...
        rateLimitPerUser: type === 'text' && slowmode ? Math.round(slowmode) : undefined, // 0 (seconds)
...
function parseSlowmode(client, message, slowmode) {
    if (!isNaN(slowmode)) {
        if (slowmode < 0 || slowmode > 21600) return message.channel.send(`${client.em.xmark} The slowmode must be between \`0\` and \`21600\` seconds`);
        else return slowmode;
    }
    if (!isNaN(slowmode.slice(0, -1))) {
        ...
        if (unit === 'm') {
            ...
            if (slowmode < 0 || slowmode > 21600) return message.channel.send(`${client.em.xmark} The slowmode must be between \`0\` and \`360\` minutes`);
            else return slowmode;
        };
        if (unit === 'h') {
            slowmode = slowmode * 60 * 60;
            if (slowmode < 0 || slowmode > 21600) return message.channel.send(`${client.em.xmark} The slowmode must be between \`0\` and \`6\` hours`);
            else return slowmode;
        };
