export default function loadPaginator(client) {
    client.paginate = (message, pages, pagesCount, timeout, startPage) => {
        const authorAvatar = message.author.displayAvatarURL({ dynamic: true, format: 'png' });
        message.channel.send(pages[startPage || 0].setFooter(`${(startPage || 0) + 1}/${pagesCount}`, authorAvatar)).then(msg => {
            msg.react('⏮').then(() => {
                msg.react('⏪').then(() => {
                    msg.react('⬅').then(() => {
                        msg.react('🛑').then(() => {
